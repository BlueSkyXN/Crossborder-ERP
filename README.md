...
| `MEMBER_PASSWORD_RESET_TOKEN_TTL_MINUTES` | `30` | 会员找回密码 token 有效期 |
| `MEMBER_PASSWORD_RESET_EXPOSE_TOKEN` | `true` | 本地/demo/test 返回 `dev_reset_token`；生产应关闭并接真实通知通道 |
...
当前 CI 会在 PR 和 `main` push 上执行 Agent 证据门禁、后端 check/OpenAPI/pytest、三端前端 lint/build，以及 `Browser Smoke` 三端登录、关键页面 smoke、包裹预报/入库/回看旅程和财务/客服跨面板旅程。
...
- 后端文件上传会校验扩展名、MIME 和基础内容签名，拦截伪装图片/PDF/XLS/XLSX；该能力不替代病毒扫描、缩略图、EXIF 清理或对象存储安全策略。
...
- 会员可在未登录状态请求找回密码并用一次性 reset token 重置密码；本地/demo/test 可返回 `dev_reset_token`，真实短信/邮件通知通道仍未接入。
...
- 通过同一临时 SQLite 浏览器会话覆盖 User Web 线下汇款、Admin Web 汇款审核入账、User Web 创建客服工单、Admin Web 回复工单和 User Web 回看客服回复。
...
- 本地文件上传、元数据、鉴权下载、包裹图片、线下汇款凭证、消息附件引用和基础内容签名校验已完成；对象存储、缩略图、病毒扫描、EXIF 清理和真实打印硬件接入后续补齐。
...
- 浏览器级 smoke 已覆盖三端登录、关键页面基础可用性、包裹预报/入库/回看旅程，以及财务汇款审核和客服工单回复跨面板旅程；视觉回归、组件级测试和所有复杂业务路径仍需后续增强。
