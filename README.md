...
- 后台 `/dashboard` 和 `/roles` 已使用真实接口与真实角色数据；角色创建、编辑和权限分配已完成，角色删除、后台用户分配角色和 create/update/delete 更细粒度 RBAC 仍属后续增强。
- 后台关键写操作审计日志已完成基础覆盖，并支持 CSV 导出和显式本地留存清理命令；外部 SIEM、自动告警、审计导出审批和更细粒度 RBAC 拆分仍属后续生产化增强。
