...
- 后台 `/dashboard`、`/roles` 和 `/admin-users` 已使用真实接口与真实数据；角色创建、编辑、权限分配、管理员创建、启停、密码重置和角色分配已完成，角色/管理员删除和 create/update/delete/export 更细粒度 RBAC 仍属后续增强。
- 后台关键写操作审计日志已完成基础覆盖，并支持 CSV 导出和显式本地留存清理命令；外部 SIEM、自动告警、审计导出审批和业务按钮级 RBAC 拆分仍属后续生产化增强。
