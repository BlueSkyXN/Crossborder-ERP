...
- 后台 `/dashboard` 使用 `GET /api/v1/admin/dashboard` 返回当前管理员可见模块的真实聚合数据；`/roles` 读取真实角色权限列表。角色编辑和细粒度 RBAC 后续单独增强。
...
线上支付不能作为 P0 已完成功能演示。P0 支持会员注册、账户设置、后台真实 dashboard/roles 面板、后台人工充值、用户线下汇款提交、财务人工审核入账、余额支付、手工代购、人工采购状态推进、基础客服工单、后台会员状态管理、无主包裹人工审核认领、内容 CMS 发布展示和 CSV / `.xlsx` 批量预报导入/导出。
