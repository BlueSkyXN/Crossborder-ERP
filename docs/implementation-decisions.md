...
| 文件内容安全 | 基础文件头校验不能发现病毒、恶意图片载荷或 EXIF 泄露 | 当前拦截明显扩展名/MIME/文件头不一致和 CSV NUL 二进制内容；CSV 导出已做公式样式字段转义；病毒扫描、EXIF 清理、图片解码和导出审批后续单独做 |
...
按 `docs/ai-dev-baseline/agent-execution/current-state.yaml` 推进。当前任务图已完成到 `CSV-EXPORT-SAFE-001`，后续如果继续收敛生产级差距，应单独确认下一张任务卡：
