...
问题：`AUDITLOG-001` 已完成后台 `/api/v1/admin/**` 写操作请求级审计、财务高风险服务层审计和 Admin Web 查询入口；`AUDIT-RETENTION-001` 已补脱敏 CSV 导出和显式本地留存清理命令；`RBAC-ROLES-001`/`RBAC-ADMIN-USERS-001` 已补角色和管理员账号写权限；`RBAC-BUSINESS-ACTIONS-001` 已补业务模块级 `*.manage` / `*.export` 权限。但尚未接外部 SIEM、自动归档、告警规则、导出审批或按每个 create/update/delete 子动作进一步分权。
...
问题：`FILE-001` 已完成本地文件上传、元数据、鉴权下载、大小/MIME/扩展名限制和包裹图片引用，`STORAGE-CLEANUP-001` 已补本地软删除文件清理命令，但尚未接对象存储、CDN、缩略图、病毒扫描和图片处理。
影响：可以支撑 SQLite-first 本地验收和后续汇款/消息/内容凭证，但不能声明生产对象存储能力已完成。
当前临时处理：使用本地 `MEDIA_ROOT`，media 目录必须持久化且不提交 git；`purge_deleted_files --older-than-days N --dry-run` 可预演软删除文件清理。
...
问题：计费公式、首发国家/渠道、短信/邮件验证码、找回密码、微信登录、多语言、真实打印模板版式和硬件接入等仍为 `TODO_CONFIRM`。
影响：不能作为最终商业规则上线。
...
- 在现有模块级业务 action 权限基础上，补角色/管理员删除，并按需要继续拆分 create/update/delete 子权限和审批流。
...
- 短信/邮件验证码、找回密码、微信登录、多语言、内容审批流和更复杂客服能力。
