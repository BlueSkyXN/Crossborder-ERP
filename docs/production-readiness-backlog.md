...
`PARCEL-CLAIM-001` 是当前下一项，因为后台会员管理已补齐，无主包裹用户查询/认领仍是 ChatGPT/Gemini 用户端报告中的 P1 缺口。

建议 `PARCEL-CLAIM-001` 范围：

- 后端补用户侧无主包裹列表/搜索/认领 API，展示信息必须脱敏。
- 后台补认领审核，通过后关联会员并转入包裹流程。
- 增加防抢认领事务测试、权限隔离测试和脱敏展示测试。
- User Web/Mobile H5 增加无主包裹入口和认领表单。
- 继续保持 SQLite-first，不接短信、邮件或外呼。
