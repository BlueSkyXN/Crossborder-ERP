...
`PAYABLE-001` 是当前下一项，因为发货批次/转单/打印模板数据基础已补齐，供应商、成本类型和应付仍是源报告里的核心财务缺口。

建议 `PAYABLE-001` 范围：

- 新增供应商、成本类型、应付款和核销基础模型/API。
- 明确应付与现有会员钱包/应收支付单的边界。
- Admin Web 增加供应商和应付基础入口。
- 保持 SQLite-first，并补金额精度、状态转换和权限测试。
