...
| 后台后端服务 | ChatGPT Admin L1005-L1023 要求 RBAC、文件、会员、消息、包裹、运单、批次、转单、财务应收/应付、内容、打印、操作日志 | 部分完成；文件、消息、内容、批次、转单和打印模板数据基础已补，应付和审计仍未完成 |
...
| 用户 Web 第二阶段 | ChatGPT User Web L795-L803 要求批量导入、直邮、无主包裹、线下汇款、推广返利、积分、物流对接、帮助中心 | 线下汇款、无主包裹认领、帮助中心和 CSV 批量预报导入基础已补；推广返利、积分、物流对接仍缺，Excel 解析仍后续 |
| 用户 Web 后端任务 | ChatGPT User Web L879-L902 要求账户、仓库、商品、购物车、订单、代购、包裹、运单、支付、财务、汇款、积分、推广、地址、消息、无主、文件、审计 | P0 主链路、地址、文件、汇款、消息工单、无主用户认领已完成；积分/推广/审计仍缺 |
...
| Gemini Admin 核心模块 | Gemini Admin L168-L175 明确 WMS、会员、商品、应收、应付、图片、网站管理、基础设置 | 应收/商品/WMS/网站管理基础已补；应付、生产图片增强仍缺 |
| Gemini User Web 需求 | Gemini User Web L290-L303 明确 dashboard、商品、手工代购、预报、包裹、无主、运单、流水、充值、推广、地址、客服 | 部分完成；地址、流水、线下汇款、客服工单和无主用户认领已补，推广仍缺 |
...
| 文件上传 | Admin 后端任务要求图片上传、凭证上传、Excel 导入、模板下载；User Web 后端任务要求文件服务 | `FILE-001` 已补本地上传、元数据、大小/MIME/扩展名限制、鉴权下载和包裹图片引用；`IMPORT-001` 已复用 `IMPORT_FILE` 支撑 CSV 导入；对象存储、缩略图、病毒扫描、Excel 解析仍后续 | `FILE-001`/`IMPORT-001` 已完成基础；对象存储/Excel 增强留给后续 |
...
| 批量导入/导出 | Admin/User Web 多处要求 Excel 导入、导出、模板下载 | `IMPORT-001` 已补包裹预报 CSV 模板下载、用户上传 `IMPORT_FILE` 后批量导入、行级错误明细、导入结果记录、会员包裹导出和后台 RBAC 包裹导出；`.xls/.xlsx` 解析仍未实现 | `IMPORT-001` 已完成基础；Excel parser 后续 |
...
1. `PAYABLE-001`：供应商、成本类型和应付基础。
2. `GROWTH-001`：积分、推广和返利基础规则。
3. Excel 原生解析增强：如业务确认必须支持 `.xlsx`，再并入后续导入增强任务，引入受控依赖并补回归测试。
4. 完整浏览器旅程增强：在现有 `npm run e2e:browser` 基础上，逐步覆盖更多真实业务流、视觉回归和组件级测试。
