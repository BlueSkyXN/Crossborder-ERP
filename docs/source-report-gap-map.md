...
| 用户 Web 第二阶段 | ChatGPT User Web L795-L803 要求批量导入、直邮、无主包裹、线下汇款、推广返利、积分、物流对接、帮助中心 | 线下汇款、无主包裹认领和帮助中心已补；批量导入、推广返利、积分、物流对接仍缺 |
...
1. `IMPORT-001`：批量导入/导出基础，复用 `FILE-001` 的上传能力。
2. `QA-BROWSER-001`：在不污染本机环境的前提下引入浏览器级 E2E。
3. `SHIP-BATCH-001`：发货批次、转单号和打印模板数据。
4. `PAYABLE-001`：供应商、成本类型和应付基础。
5. `GROWTH-001`：积分、推广和返利基础规则。
