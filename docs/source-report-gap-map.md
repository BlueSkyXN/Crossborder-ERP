...
| 用户 Web 第二阶段 | ChatGPT User Web L795-L803 要求批量导入、直邮、无主包裹、线下汇款、推广返利、积分、物流对接、帮助中心 | 线下汇款、无主包裹认领、帮助中心、CSV 批量预报导入、积分/推广/返利基础已补；物流对接和 Excel 解析仍后续 |
| 用户 Web 后端任务 | ChatGPT User Web L879-L902 要求账户、仓库、商品、购物车、订单、代购、包裹、运单、支付、财务、汇款、积分、推广、地址、消息、无主、文件、审计 | P0 主链路、地址、文件、汇款、消息工单、无主用户认领、积分/推广基础已完成；审计日志仍缺 |
...
1. Excel 原生解析增强：如业务确认必须支持 `.xlsx`，再并入后续导入增强任务，引入受控依赖并补回归测试。
2. 完整浏览器旅程增强：在现有 `npm run e2e:browser` 基础上，逐步覆盖更多真实业务流、视觉回归和组件级测试。
3. 审计日志和生产化运维边界：补后台关键操作日志、PostgreSQL/MySQL/Redis 真实验证计划、对象存储和部署验证。
